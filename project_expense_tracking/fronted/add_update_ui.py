...
def add_update_tab():
    ...
    if response.status_code==200:
        ...
        st.error("Failed to retrive")
        existing_expenses=[]


    categories=["Rent","Food","Shopping","Entertainment","glasses","vehicle","Other"]
    ...
    with st.form(key="Expense_form"):
        ...
        if submit_button:
               ...
               requests.post(f"{API_URL}/expenses/{selected_date}",json=filtered_expense)
               
               if response.status_code==200:
                   st.success("Expense updated successfully!")
               else:
                   st.error("Failed to udate expenses.")